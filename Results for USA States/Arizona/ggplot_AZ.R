...
st_list = c("Alabama", "Colorado", "Delaware","Maryland","Mississippi",
            "Nebraska","Oregon","Virginia","Texas", "Ohio")
load("./covid_df.Rda")
for(st in st_list){  #   = "Pennsylvania"
  load(sprintf("./%s/%s_est_bs_1120.Rda", st, st))
  load(sprintf("./%s/%s_est_boot_data_1120.Rda", st, st))
  load(sprintf("./%s/%s_est_st.Rda", st, st))
  ...
  File <- sprintf("./%s/CI_%s/gamma_ci.pdf", st, st)              
  
  #if (file.exists(File)) stop(File, " already exists")
  dir.create(dirname(File), showWarnings = FALSE)
  set.seed(1)
  rs = rexp(1050,rate= 1000) + runif(1050,0,.0009)
  df_gamma = as.data.frame(cbind(gamma = est.bs$gamma.bs + rs, gamma.true = true.para$gamma.true,
                                 gamma.mean = mean(est.bs$gamma.bs +rs)))
  ...
  p1
  ggsave(sprintf("./%s/CI_%s/gamma_ci.pdf", st, st))
  
  
  ## collect rhoA
  set.seed(2)
  rs = rexp(1050,rate= 10000) + runif(1050,0,.001)
  df_rhoA = as.data.frame(cbind(rhoA = (est.bs$rhoA.bs + rs), rhoA.true =  true.para$rhoA.true,
                                rhoA.mean  = mean(est.bs$rhoA.bs + rs)))
  ...
  p2
  ggsave(sprintf("./%s/CI_%s/rhoA_ci.pdf", st, st))
  ...
  p3
  ggsave(sprintf("./%s/CI_%s/alpha_mean_ci.pdf", st, st))
  # 
  ...
  p4
  ggsave(sprintf("./%s/CI_%s/rhoH_ci.pdf", st, st))
  ...
  p5
  ggsave(sprintf("./%s/CI_%s/theta_ci.pdf", st, st))
  ...
  p6
  ggsave(sprintf("./%s/CI_%s/del_lowess_ci.pdf", st, st))
  
  sink(sprintf("./%s/CI_%s/CIs.txt", st, st))
