...
estimation_module = function(dat, numBlock, wd.length, bw_lowess, boot = FALSE){
  ...
  if(boot == FALSE){
    ...
    File <- sprintf("../Results for USA States/%s/%s_updated/%s_plot_gamma.eps", dat$st, dat$st,dat$st)              
    ...
    #ggsave(sprintf("../Results for USA States/%s/%s_updated/%s_plot_gamma.eps",  dat$st,  dat$st,dat$st))
    ...
    #ggsave(sprintf("../Results for USA States/%s/%s_updated/%s_plot_rhoA.eps",  dat$st,  dat$st,dat$st))
...
plot_for_estimation = function(st, numBlock, wd.length,bw_lowess){
  ...
  #load(sprintf("../Results for USA States/%s/%s_updated/%s_est_st.Rda", st, st, st))
  
  sink(sprintf("../Results for USA States/%s/%s_updated/%s_est_st.Rda", st, st, st))
  save(est_st, file = sprintf("../Results for USA States/%s/%s_updated/%s_est_st.Rda", st, st, st))
  sink()
...
# est_st_old = est_st
# n_old = length(est_st_old$est_para$est$sqrt.alpha.kappa.t)
# R0_numer = (est_st_old$est_para$est$sqrt.alpha.kappa.t)^2 * unique(data.st$var$pop19)
# R0_denom = (est_st_old$est_states$S + est_st_old$est_states$A + est_st_old$est_states$R) * 
...
# plot((1:length(R0)),R0, type = 'l')
# 
# 
# 
# load("../Results for USA States/Utah/Utah_updated/Utah_est_st.Rda")
# est_st_new = est_st
# n_new = length(est_st_new$est_para$est$sqrt.alpha.kappa.t)
# R0_numer_new = (est_st_new$est_para$est$sqrt.alpha.kappa.t)^2 * unique(data.st$var$pop19)
# R0_denom_new = (est_st_new$est_states$S + est_st_new$est_states$A + est_st_new$est_states$R) * 
#   (est_st_new$est_para$est$theta.hat.t + est_st_new$est_para$est$gamma.hat + est_st_new$est_para$est$rhoA.hat)
# R0_new = R0_numer_new/R0_denom_new[1:length(R0_numer_new)]
# 
# lines((1:length(R0_new)),R0_new, type = 'l', col = 'red')
# abline(h= 1)
# 
