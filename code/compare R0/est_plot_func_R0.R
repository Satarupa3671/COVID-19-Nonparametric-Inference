...
estimate_parameters = function(gamma.hat, rhoA.hat, var, shift, numBlock, wd.length, TimeBlock, bw_lowess){
  ...
  theta.hat.t = pmax(phi.hat.smooth[1:n_eff] * TtoH[1:n_eff],0.0001) # estimate of theta(t) [confirmation fraction]
  
  delC.low = lowess(delC,f=bw_lowess)$y  # lowess smoothing of Delta(C(t))
  CIR.t = diff(delC.low)/pmax(delC.low[-length(delC.low)],0.0001)   # 210 Crude Infection Rate =  \Delta^2 C(t)/\Delta C(t)
  
  RCCF.t = lowess(diff(theta.hat.t)/pmax((theta.hat.t[1:length(diff(theta.hat.t))]+gamma.t),0),f=1/8)$y  # relative change in confirmation fraction eta(t) = theta(t)+gamma
  #210
  ...
  #210
  A.hat.t = delC/pmax((theta.hat.t[1:length(delC)] + gamma.t),.0001)  # fitted value of A(t)
  #211
...
plot_est = function(var,  raw_data , est_st, dates,bw_lowess){
  ...
  sink(sprintf("../Results for USA States/%s/%s_updated/%s_opt_gamma_rhoA.txt", st, st, st))
  ...
  ggsave(file = sprintf("../Results for USA States/%s/%s_updated/%s_validaton_H.pdf", st, st, st))
  ...
  #RCCF
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_RCCF.pdf", st, st, st))
  ...
  CIR_NIR = ggplot(df_CIR_NIR, aes(x=dates1, y=CIR.t))+
    ...
    theme(legend.position= "none",# c(.4,.85),
          ...
          legend.text=element_text(size=24))
  #CIR_NIR
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_NIR_CIR.pdf", st, st, st))
  ...
  CFR_plot = ggplot(df_CFR)+
    ...
    theme(legend.position= "none",# c(.4,.85),
          ...
          legend.text=element_text(size=24))
  #CFR_plot
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_CFR.pdf",st, st, st))
  ...
  #R0_plot
  ggsave(file = sprintf("../Results for USA States/%s/%s_updated/%s_R0_analogue.pdf",st, st, st))
  ...
  #Ahat_plot
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_Ahat.pdf", st, st, st))
  ...
  #New_Infect
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_NewInfect.pdf", st, st, st))
  ...
  doub_rt_plot = ggplot(df_doub, aes(dates1)) + 
    ...
    theme(legend.position= "none",# c(.4,.85),
          ...
          legend.text=element_text(size=24))
  #doub_rt_plot
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_Doubling_Rate.pdf", st, st, st))
  ...
  TtoH_plot = ggplot(df_CRHD, aes(dates1)) + 
    ...
    theme(legend.position= "none",# c(.4,.85),
          ...
          legend.text=element_text(size=24))
  #TtoH_plot
  ggsave(file =sprintf("../Results for USA States/%s/%s_updated/%s_TtoH_plot.pdf", st, st, st))
